...
import com.u8.server.service.LogDataManager;
...
public class SendAgent {
    ...
    private static Logger logger = Logger.getLogger(SendAgent.class.getName());
...
//            data.put("platID", order.getPlatID());
            ...
            response.put("orderNo", order.getExtension());
...
//            response.append("sign=").append(sign);
    ...
    private static void logForOrder(UOrder order) {
        ApplicationContext applicationContext = UApplicationContext.getApplicationContext();
        LogDataManager mLogDataManager= (LogDataManager) applicationContext.getBean("logDataManager");

        UStatistics statistics = mLogDataManager.getStatisticsByAppId(order.getAppID());

        if (null == statistics) {
            logger.error("-------->The sendAgent order statistics is null,order's appid:"+order.getAppID());
            return ;
        }

        String token = LogDataManager.getToken();
        String orderCode = mLogDataManager.logOrderData(statistics.getCid(), order, token);
        logger.debug("-------->The logOrderData,code:" + orderCode);

        if ("401".equals(orderCode)) {
            ...
            String twiceOrderCode = mLogDataManager.logOrderData(statistics.getCid(), order, twiceToken);
            logger.info("-------->logOrderData ,twiceOrderCode:"+twiceOrderCode+", twiceToken:"+twiceToken);
        }
