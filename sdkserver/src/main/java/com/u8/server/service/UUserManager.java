...
public class UUserManager {
    ...
    //根据渠道用户ID获取用户信息
    public List<UUser> getUserByCpID(String cpUserID){

        String hql = "from UUser where channelUserID = ?";
        String[] strArr= new String[]{cpUserID};
        return (List<UUser>)userDao.find(hql, strArr, null);
...
//        Log.d("The token is "+token +"; the right token is "+tokenRight);
    ...
    public UUser generateOldBSUser(UChannel channel, SDKVerifyResult cpUserInfo){

        UUser user = new UUser();
        user.setAppID(channel.getAppID());
        user.setChannelID(channel.getChannelID());
        user.setName(System.currentTimeMillis() + "oldBsUser");
        user.setChannelUserID(cpUserInfo.getUserID());
        user.setChannelUserName(cpUserInfo.getUserName() == null ? "" : cpUserInfo.getUserName());
        user.setChannelUserNick(cpUserInfo.getNickName() == null ? "" : cpUserInfo.getNickName());
        Date now = new Date();
