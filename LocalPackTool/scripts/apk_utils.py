...
#write sub channel id into u8_developer_config.properties
def writeSubChannel(subChannelID, decompileDir):

    developConfigFile = os.path.join(decompileDir, "assets")
    developConfigFile = os.path.join(developConfigFile, "u8_developer_config.properties")
    if not os.path.exists(developConfigFile):
        log_utils.error("u8_developer_config.properties not exists")
        return 1

    f = open(developConfigFile, "r")
    fstr = ""
    for line in f.readlines():
        if "U8_Sub_Channel" not in line:
            fstr = fstr + line

    f.close()

    fstr = fstr + "U8_Sub_Channel=" + subChannelID + "\n"
    fstr = fstr.encode('UTF-8')

    f = open(developConfigFile, "wb")
    f.write(fstr)
    f.close()
...
def doSDKScript(channel, decompileDir, packageName, sdkTempDir):
    ...
    log_utils.info("hwhwhwhwhwhw %s,%s,%s,%s", channel, decompileDir, packageName, sdkTempDir)
...
def addLocalJars(workDir, decompileDir):
    ...
    file_utils.copy_files(u8sdkJarPath, targetPath)
...
def getOutputApkName(game, channel, packageName, decompileDir):
    ...
    if params != None:
             for param in params:
                 if param['name'] == 'UniqueSerial':
                        BSUniqueSerial = param['value']
                        
                 if param['name'] == 'ServerName':
                        BSServerName = param['value']
    ...
    if BSUniqueSerial != None:
        formatStr = formatStr.replace('{UniqueSerial}', BSUniqueSerial)
    if BSServerName != None:
        formatStr = formatStr.replace('{ServerName}', BSServerName)
